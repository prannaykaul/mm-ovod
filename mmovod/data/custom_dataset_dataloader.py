...
def _custom_train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None):
    ...
    if 'MultiDataset' in sampler_name:
        dataset_dicts = get_detection_dataset_dicts_with_source(
            ...
            ann_types=cfg.DATALOADER.DATASET_ANN,
        )
    else:
...
def filter_images_with_only_crowd_annotations_detic(dataset_dicts):
    """
    ...
    dataset_dicts = [x for x in dataset_dicts if valid(x["annotations"], x['ann_type'])]
...
def get_detection_dataset_dicts_with_source(
    dataset_names, filter_empty=True, min_keypoints=0, proposal_files=None, ann_types=[],
):
    ...
    for source_id, (dataset_name, dicts) in enumerate(zip(
            dataset_names, dataset_dicts)):
        assert len(dicts), "Dataset '{}' is empty!".format(dataset_name)
        for d in dicts:
            d['dataset_source'] = source_id
            d['ann_type'] = ann_types[source_id]
